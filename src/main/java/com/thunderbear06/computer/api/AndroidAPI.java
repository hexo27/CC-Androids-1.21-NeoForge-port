...
public class AndroidAPI implements ILuaAPI
{
    ...
    */
    ...
    public final MethodResult jump() throws LuaException
    {
        if (missingFuel())
            throw new LuaException("Android requires fuel.");

        this.brain.getAndroid().jump();
        return MethodResult.of();
    }
    ...
    public final MethodResult breakBlock(IArguments args) throws LuaException
    {
        ...
        if (!pos.closerThan(this.brain.getAndroid().blockPosition(), 100))
            return MethodResult.of(true, "Block position must be within a 100 block radius");

        if (this.brain.getAndroid().level().isOutsideBuildHeight(pos))
    ...
    public final MethodResult storeItem(int index)
    {
        ...
        this.brain.getAndroid().setStackInHand(InteractionHand.MAIN_HAND, itemStack);
    ...
    {
        int size = brain.getAndroid().inventory.getContainerSize()-1;
        ...
        this.brain.getAndroid().setStackInHand(InteractionHand.MAIN_HAND, storedItemstack);
    ...
    public final MethodResult getSlotInfo(int index)
    {
        ...
        return MethodResult.of(false, storedStack.getHoverName().getString());
    }
    ...
    public final MethodResult refuel(IArguments args) throws LuaException
    {
        ...
        if (heldStack.isEmpty())
            return MethodResult.of(true, "InteractionHand is empty");
        ...
        this.brain.getAndroid().setStackInHand(InteractionHand.MAIN_HAND, heldStack);
    ...
    */
    ...
    {
        BlockPos pos = this.brain.getAndroid().blockPosition();
        ServerLevel world = (ServerLevel) this.brain.getAndroid().level();

        return MethodResult.of(brain.getModules().sensorModule.getBlocksOfType(pos, this.brain.getAndroid().getEyePosition(), world, type));
    }
